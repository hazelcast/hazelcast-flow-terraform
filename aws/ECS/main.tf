...
module "hazelcast_flow" {
  source                                  = "./modules/hazelcast-flow"
  region                                  = var.region
  ...
  domain_name                             = local.environment_domain_name
  database_host                           = aws_db_instance.flow-db.address
  database_username                       = aws_db_instance.flow-db.username
...
resource "aws_db_instance" "flow-db" {
  ...
  final_snapshot_identifier = "flow-pre-delete-snapshot-${var.environment}-${random_string.database_instance_suffix.result}"
  db_subnet_group_name      = aws_db_subnet_group.flow-platform.name
